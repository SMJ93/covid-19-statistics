...
export default async function getData() {
  ...
  if (response.status === 200) {
    ...
    if (json.data && json.data.length > 0) {
      const formattedData = json.data.map(
        ({name, code, population, updated_at, today, latest_data}) => {
          ...
          const totalDeathsPerPopulation = hasDeathAndPopulationState
            ...
            : 0;
          const totalDeathsPerPopulationPercentage =
            totalDeathsPerPopulation * 100;
          const totalDeathsPerMillion = totalDeathsPerPopulation * 1000000;
          const totalDeathsPerHundredThousand =
            totalDeathsPerPopulation * 100000;
          ...
          // @todo handle 0 population / other 0 values
          return {
            name,
            code,
      ...
      // Sorted by totalDeathsPerPopulation
